...
import androidcustomcalendar.adms.com.calender.ui.adapter.event.EventAdapter;
...
public class AllEventFragment extends BaseFragment implements EventAdapter.OnEventItemClickListener, View.OnClickListener, DeleteFragment.OnDeleteConfirmListener {
    ...
    public void onDeleteClick(final int position, final Event event) {
        ...
        showSnackbar("Berhasil dihapus ^ _ ~", "Batalkan", v -> {
            mEventDao.add(event);
            mEvents.add(event);
            Collections.sort(mEvents, mComparator);
    ...
    private void deleteComplete() {
        ...
        showSnackbar("Menghapus acara yang dicoret. "," Saya ingin pulihkan ", v -> {
    ...
    private void deleteAll() {
        ...
        showSnackbar("Semua acara telah dihapus. "," Saya ingin pulihkan ", v -> {
            mHandler.sendEmptyMessage(WAIT);
            for (Event event : temp) {
                mEventDao.add(event);
            }
    ...
    private static class EventHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case WAIT:
                    fragment.mProgressDialog = ProgressDialog.show(fragment.mHostActivity, null, "Pulihkan, harap tunggu ...");
                    break;
                case CANCEL:
